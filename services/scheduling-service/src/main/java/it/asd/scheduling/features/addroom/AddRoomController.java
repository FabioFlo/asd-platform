    ...
    public ResponseEntity<?> add(
        ...
        return switch (handler.handle(effectiveCmd)) {
            ...
            case AddRoomResult.VenueNotFound v -> {
                ...
                pd.setDetail("Venue not found: " + v.venueId());
                yield ResponseEntity.status(HttpStatus.NOT_FOUND).body(pd);
            }

            case AddRoomResult.DuplicateName d -> {
                var pd = ProblemDetail.forStatus(HttpStatus.UNPROCESSABLE_ENTITY);
                pd.setType(URI.create("https://asd.it/errors/duplicate-room-name"));
                pd.setDetail("Room name already exists in this venue: " + d.nome());
